...
import { useSpuitschriftEntries, useParcels, useInvalidateQueries } from '@/hooks/use-data';
...
function ActionsMenu({ entry, onAction }: { entry: SpuitschriftEntry; onAction: () => void }) {
    ...
    const { invalidateSpuitschrift, invalidateLogbook } = useInvalidateQueries();
...
function ChronologicalView({ entries, allParcels, onAction }: { entries: SpuitschriftEntry[], allParcels: SprayableParcel[], onAction: () => void }) {
            ...
            <CardContent>
                <Tabs defaultValue="chronological">
                    ...
                    <TabsContent value="chronological" className="mt-6">
                        {entries.length > 0 ? (
                            <ChronologicalView entries={entries} allParcels={allParcels} onAction={handleAction} />
                        ) : (
                            <EmptyState
                                icon={BookOpen}
