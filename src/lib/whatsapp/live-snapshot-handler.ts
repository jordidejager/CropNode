...
// ============================================================================
// Text builder (shared by WhatsApp handler and MCP)
// ============================================================================

export async function buildLiveSnapshotText(userId: string): Promise<string | null> {
...
export async function handleLiveSnapshot(
  ...
  try {
    ...
    const msg = await buildLiveSnapshotText(userId);
    if (!msg) {
      const noStations =
        '📡 Je hebt nog geen fysieke weerstations gekoppeld.\n\n' +
        'Wil je de _forecast_ verwachting in plaats daarvan? Stuur "weersverwachting".';
      await sendTextMessage(metaPhone, noStations);
      await logMessage({ phoneNumber, direction: 'outbound', messageText: noStations });
      return;
    }
