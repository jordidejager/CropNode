...
export let middelMatrix: Middel[] = [
...
// This is a temporary in-memory solution. For persistence, you'd save this to a database.
export function addMiddelToMatrix(newMiddelen: Middel[]) {
  // Simple check to avoid duplicates for this example
  for (const newMiddel of newMiddelen) {
      const exists = middelMatrix.some(
      ...
      if (!exists) {
          middelMatrix = [newMiddel, ...middelMatrix];
      }
  }
}
