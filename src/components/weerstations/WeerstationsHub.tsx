...
import { Radio, Plus, Settings, Zap, RefreshCw } from 'lucide-react';
...
function Header({
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      await qc.invalidateQueries({ queryKey: ['physical-stations'] });
    } finally {
      // Brief spinner so the click feels acknowledged
        ...
        {hasStations && (
          <>
            <button
              ...
              disabled={refreshing}
              className="inline-flex items-center gap-1.5 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white/90 hover:bg-white/10 transition-colors px-2.5 py-2 text-xs font-semibold disabled:opacity-60"
              title="Ververs metingen"
            >
