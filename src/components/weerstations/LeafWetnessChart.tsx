...
import {
  aggregateDailyLeafWetness,
  aggregateHourlyLeafWetForDay,
  type DailyLeafWet,
...
/**
 * Leaf wetness duration per day for one leaf sensor — the agronomically useful
 * read for scab pressure ("how many hours was the leaf wet each day"). Same
 * shape as the daily rain chart: bars per day, click to expand the hour-by-hour
 * breakdown, with a big "vandaag tot nu toe" counter.
 */
export function LeafWetnessChart({
  stationId,
  stationLabel,
}: {
  stationId: string;
  ...
  const daily = useMemo(
    () => aggregateDailyLeafWetness(measurements ?? [], range === '7d' ? 7 : 30),
    [measurements, range]
  );
  const today = useMemo(
    () => daily.find(d => d.isToday)?.wetHours ?? 0,
    [daily]
  );
  const total = useMemo(
    () => Math.round(daily.reduce((s, d) => s + d.wetHours, 0) * 10) / 10,
    [daily]
  );

  const hourly = useMemo(
    () => (openDay ? aggregateHourlyLeafWetForDay(measurements ?? [], openDay) : null),
    [measurements, openDay]
  );

  const maxHours = Math.max(...daily.map(d => d.wetHours), 1);
    ...
    <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
      ...
      <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Bladnat per dag</h3>
            <p className="text-xs text-white/40">
              {stationLabel ? `${stationLabel} · ` : ''}uren nat · klik een dag voor uur-detail
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/15 to-transparent p-3.5">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-sm text-white/40">uur nat</span>
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3.5">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-sm text-white/40">uur</span>
          </div>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              ...
              onClick={(state) => {
                const payload = state?.activePayload?.[0]?.payload as DailyLeafWet | undefined;
            ...
            >
              ...
              <Bar dataKey="wetHours" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                {daily.map(d => (
                  <Cell
                    key={d.date}
                    cursor="pointer"
                    fill={
                      d.date === openDay
                        ...
                        : d.isToday
                          ? '#10b981'
                          : `rgba(52,211,153,${0.35 + 0.5 * (d.wetHours / maxHours)})`
                    }
                  />
                ))}
              </Bar>
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.02] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] text-white/40">
              · {(hourly.reduce((s, h) => s + h.wetMinutes, 0) / 60).toFixed(1)} uur nat totaal
            </span>
            <button
          ...
          <div className="h-[140px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={hourly} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                ...
                <Bar dataKey="wetMinutes" fill="#34d399" radius={[3, 3, 0, 0]} isAnimationActive={false} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
...
function DailyTooltip({
  ...
  active?: boolean;
  payload?: Array<{ payload: DailyLeafWet }>;
}) {
  ...
  return (
    <div className="rounded-lg bg-slate-950/95 border border-white/10 shadow-xl px-3 py-2">
      ...
      <div className="text-sm font-bold text-emerald-300">{p.wetHours.toFixed(1)} uur nat</div>
    </div>
  );
}
...
function HourlyTooltip({
  ...
  active?: boolean;
  payload?: Array<{ payload: { hour: number; wetMinutes: number } }>;
}) {
  if (!active || !payload || payload.length === 0) return null;
  const p = payload[0]!.payload;
  return (
    <div className="rounded-lg bg-slate-950/95 border border-white/10 shadow-xl px-3 py-2">
      ...
      <div className="text-sm font-bold text-emerald-300">{p.wetMinutes} min nat</div>
    </div>
  );
}
