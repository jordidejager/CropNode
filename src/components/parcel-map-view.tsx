...
export function ParcelMapView({ parcels }: ParcelMapViewProps) {
    ...
    // Create a unique key based on the parcels to force re-render.
    // This solves the "Map container is already initialized" error when switching tabs.
    const mapKey = useMemo(() => parcels.map(p => p.id).join('-'), [parcels]);
        ...
        <Card>
            ...
            <CardContent>
                <div className="h-[600px] w-full rounded-md border overflow-hidden">
                     <MapContainer key={mapKey} center={mapCenter} zoom={8} style={{ height: '100%', width: '100%' }}>
                        <TileLayer
