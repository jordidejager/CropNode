...
- ✅ `src/lib/whatsapp/spray-inbox.ts`: `handleSprayInboxMessage` (insert `logbook` + ack) + `processSprayDraft` (pipeline + werkzame-stof→merk uit eigen historie + laatste dosering + user_preferences-substitutie) via Next `after()`. Vangnet-cron `/api/cron/spray-inbox` (1x/dag — `*/15` liet de Vercel-deploy falen: Hobby-plan staat alleen dagelijkse crons toe, zie ook `cf195b4`).
